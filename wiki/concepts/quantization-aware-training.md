...
Quantization-aware training (QAT) aims to produce a model with high quality when run in low precision at inference. During QAT, the model simulates low-precision (e.g., 8-bit) behavior while training, letting it learn to produce high-quality outputs once actually quantized (AIE p.331). This addresses the risk that post-training quantization degrades a model's quality.
