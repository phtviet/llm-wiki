...
Including evaluation examples in the judge's prompt is one mitigation: Zheng et al. (2023) found this raised [[gpt-4|GPT-4]]'s consistency from 65% to 77.5%. However, high consistency does not imply high accuracy -- a judge can consistently make the same mistake -- and longer prompts from added examples increase inference cost; in Zheng et al.'s experiment, adding more examples quadrupled GPT-4 spending (AIE p.141).
