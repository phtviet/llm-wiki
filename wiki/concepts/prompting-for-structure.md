...
Prompting is the first line of action for getting a model to produce structured outputs: instructing the model to generate in a given format. Whether the model complies depends on its instruction-following capability and the clarity of the instruction, and there is no guarantee it will always follow the instruction (AIE p.101).
