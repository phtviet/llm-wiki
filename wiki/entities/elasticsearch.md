...
Elasticsearch (Shay Banon, 2010), built on top of Lucene, is a term-based retrieval solution using a data structure called an inverted index: a dictionary mapping terms to the documents that contain them, enabling fast retrieval given a term. The index may also store term frequency and document counts to support TF-IDF-style scoring (AIE p.259).
